...
<div class="post_card">
	<p style="text-align: center; padding: 20px; color: #a6b4c8;margin: 0px;">@lang('trans.nothingToShow')</p>
</div>
@else
<?php $sent_toId_index = 0;$commentsCount_index = 0; ?>
@foreach($feedbacks as $feedback)
<div class="post_card" @if(App::isLocale('ar')) style="text-align: right;direction: rtl" @endif data-post="{{ $feedback->pid }}" @if($feedback->read == 0) style="border-top: 0px solid red;" @endif>
	...
	<div class="post_card_content" data-postContent="{{ $feedback->pid }}">
		...
		@if(Auth::user())
			...
			@endif
			<label @if($feedback->to_id != Auth::user()->uid) style="margin: 0" @endif class="fb-comment" data-comment="{{ $feedback->pid }}"><span class="far fa-comment fa-2x"></span> @lang('trans.addA_comment')</label>
		@endif
	...
	<div class="post_card_comments" data-commentCard="{{ $feedback->pid }}" data-show="0">
		<textarea dir="auto" maxlength="500" class="fb-comment-field" data-comment="{{ $feedback->pid }}" placeholder="Write a reply.." style="direction: inherit;"></textarea>
	...
	<div class="comments-card" data-cCard="{{ $feedback->pid }}">
		...
		<a style="margin: 0px 15px;font-size: 11px;" class="show-comments" data-cshow="{{ $feedback->pid }}" href="javascript:void(0);"><span class="far fa-comments fa-2x"></span> @lang('trans.showComments') {{ '('.$commentsCount[$commentsCount_index].')' }}</a>
...
<br>
@if(Auth::user())
<center><p style="color:#ffffff">100 Confessions and Apply for a Verified Badge.</p><a href="mailto:verified@pinglu.in?Subject=Want%20a%20Verified%20Badge&body=I confirm my total public post in Pinglu are more than 100 and my Profile link is https://pinglu.in/{{ Auth::user()->username }}" class="login-btn" target="_top">Apply for Badge</a></center>
